...
function LoginPage() {
  ...
  const handleSubmission = () => {
    ...
    var config = {
      method: "POST",
      url:
        "http://127.0.0.1:5000/uploader?type=user&id=" +
  ...
  const submitRatio = () => {
    axios
      .post("http://localhost:3001/ratio", {
        tempratio: tempratio,
        token: localStorage.getItem("token"),
        id: localStorage.getItem("id"),
      })
  ...
  const getMarketData = () => {
    axios
      .get("http://localhost:3001/marketdata", {})
      .then((response) => {
